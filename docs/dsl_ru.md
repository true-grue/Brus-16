...
* В текущей версии компилятора не проверяется согласованность `return` и если возвращаемое значение не используется, его все равно необходимо присвоить переменной, такой как `_`.
...
* `set_fp(address)`: установка указателя фрейма (`fp`). Функции, в которых вызывается `set_fp`, не должны использовать локальные переменные.
